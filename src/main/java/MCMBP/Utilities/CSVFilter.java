...
public class CSVFilter {
	...
	public List<CSVRecord> FilterCSV(Vector<String> Values,HashMap<String,Integer> HeadersWithIndex,List<CSVRecord> list, String Col ){
		  ...
		  if(v.contains(",")) {
			...
			for(double i=Double.parseDouble(Start) ; i <= Double.parseDouble(End) ; i=i+0.01) {
				...
				for(int frac=0; frac <10 ; ++frac) {
					if(new BigDecimal(i).setScale(2, RoundingMode.HALF_UP).remainder( BigDecimal.ONE ).compareTo(new BigDecimal("0."+frac+"0"))==0) {// if 2019.00 convert to 2019
						if(frac==0) 
